...
class ControllerCommonSeoPro extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$this->cache_data) {
			...
			$query = $this->db->query("SELECT LOWER(`old`) as 'old', `new` FROM " . DB_PREFIX . "url_oldalias ORDER BY url_oldalias_id");
			foreach ($query->rows as $row)
				$this->cache_data['old'][$row['old']] = $row['new'];
      ...
      // OCFilter end
		...
		if (!isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $keyword) {
				if(isset($this->cache_data['old'][$keyword]))
					$keyword = $this->cache_data['old'][$keyword];
				if (isset($this->cache_data['keywords'][$keyword])) {
					$rows[] = array('keyword' => $keyword, 'query' => $this->cache_data['keywords'][$keyword]);
				}
			}
			
			if(isset($this->cache_data['old'][$route]))
				$route = $this->cache_data['old'][$route];
			...
			if (count($rows) == sizeof($parts)) {
				...
				foreach ($parts as $part) {
					if(isset($this->cache_data['old'][$part]))
						$part = $this->cache_data['old'][$part];
...
} elseif (isset($this->request->get['news_id'])) {$this->request->get['route'] = 'information/news';
			} elseif(isset($this->cache_data['queries'][$route_]) and !isset($this->cache_data['keywords'][$route_])) {
...
case 'news_id':
					...
					case 'path':
						...
						$queries = (1 < count($categories)) ? array('category_id='.$categories[0],'category_id='.$categories[count($categories)-1]) : array('category_id='.$categories[0]);
