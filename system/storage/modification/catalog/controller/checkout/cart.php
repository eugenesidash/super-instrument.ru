...
class ControllerCheckoutCart extends Controller {
	public function index() {
		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$data['checkout'] = $this->url->link(isset($settings['show_checkout']) ? 'checkout/unicheckout' : 'checkout/checkout'
			, '', 'SSL');
	...
	public function add() {
		...
		if ($product_info) {
			...
			if (isset($settings['show_checkout'])) {
				$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $this->url->link('checkout/unicheckout'));
				$json['success_new'] = sprintf($this->language->get('text_success_new'), $success_image, $json['success'], $this->url->link('checkout/unicheckout'));
			} else {
			...
			$json['total_items'] = $this->cart->countProducts() != null || $this->cart->countProducts() != 0 ? $this->cart->countProducts() : 0;
