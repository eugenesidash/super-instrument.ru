...
class ControllerExtensionModule extends Controller {
	...
	public function install() {
		...
		if ($this->validate()) {
			...
			$this->load->controller(in_array(DIR_APPLICATION.'controller/module/'.$this->request->get['extension'], $files) ? 'module/'.$this->request->get['extension'] : 'extension/module/' . $this->request->get['extension'] . '/install');
	...
	public function uninstall() {
		...
		if ($this->validate()) {
			...
			//Для совместимости с 2.1
			$files = glob(DIR_APPLICATION . 'controller/module/*.php');
			$this->load->controller(in_array(DIR_APPLICATION.'controller/module/'.$this->request->get['extension'], $files) ? 'module/'.$this->request->get['extension'] : 'extension/module/' . $this->request->get['extension'] . '/uninstall');
	...
	public function getList() {
		...
		foreach ($extensions as $key => $value) {
			...
			//Для совместимости с 2.1
			if (!file_exists(DIR_APPLICATION.'controller/module/'.$value.'.php') && !file_exists(DIR_APPLICATION.'controller/extension/module/'.$value.'.php')) {
			...
			$this->load->language('extension/module/' . $extension);
					...
					foreach ($modules as $module) {
						$module_data[] = array(
								...
								'edit' => $this->url->link(in_array(DIR_APPLICATION.'controller/module/'.basename($extension).'.php', $files) ? 'module/'.$extension : 'extension/module/' . $extension, 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], 'SSL'),
					...
					$data['extensions'][] = array(
							...
							'edit' => $this->url->link(in_array(DIR_APPLICATION.'controller/module/'.basename($extension).'.php', $files) ? 'module/'.$extension : 'extension/module/' . $extension, 'token=' . $this->session->data['token'], 'SSL')
					);
				} else {
					$data['hiden'] = true;
				}
