...
class ModelModuleCustomMenu extends Model {
	...
	$this->load->model('tool/image');
			...
			foreach($query->rows as $row){
				$query1 = $this->db->query("SELECT * FROM " . DB_PREFIX . "custom_menu h LEFT JOIN " . DB_PREFIX . "custom_menu_description hd ON (h.menu_id = hd.menu_id) where hd.language_id = '" . (int)$this->config->get('config_language_id') . "' and h.parent_id='".(int)$row['menu_id']."' and  h.status=1 order by h.sort_order" );
